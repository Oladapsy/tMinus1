...
export default function DisableTwoFactorScreen() {
  ...
  const handleDisable2FA = async () => {
    ...
    try {
      ...
      showToast?.(error?.data?.message || "Failed to disable 2FA. Please check your entries.", "error");
    }
  };

  return (
    <ImageBackground source={require("@/assets/images/kyc/kycBg.png")} style={styles.backgroundImage} resizeMode="cover">
      <MySafeAreaView style={styles.safeContainer}>
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.passwordInputRow}>
            ...
            <TouchableOpacity onPress={() => setShowPassword(!showPassword)}>
              {showPassword ? (
                <EyeOpenIcon color={Colors.newSecondary} width={20} height={20} />
              ) : (
          ...
          >
            <Text style={styles.modeToggleText}>
              {isUsingRecovery ? "Use standard authenticator pin instead" : "Lost access? Use a backup recovery code"}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.footerSection}>
          <PrimaryButton
            ...
            Bgcolor={(password && code) ? Colors.red || "#FF3333" : Colors.newDark}
...
const styles = StyleSheet.create({
  ...
  safeContainer: { flex: 1, backgroundColor: "transparent", paddingHorizontal: 24, justifyContent: "space-between", paddingBottom: 24 },
  formContainer: { marginTop: 30, gap: 10 },
  inputLabel: { color: Colors.newWhite, fontSize: 13, fontFamily: FontFamily.bold, marginTop: 12, marginBottom: 4 },
  passwordInputRow: { flexDirection: "row", alignItems: "center", backgroundColor: Colors.newDark, borderRadius: 12, paddingHorizontal: 16, height: 54 },
  flexInput: { flex: 1, color: "white", fontSize: 14, fontFamily: FontFamily.regular, height: "100%" },
  standardInput: { backgroundColor: Colors.newDark, borderRadius: 12, paddingHorizontal: 16, height: 54, color: "white", fontSize: 14, fontFamily: FontFamily.regular },
  modeToggleWrapper: { marginTop: 8, alignSelf: "flex-start" },
  modeToggleText: { color: Colors.green, fontSize: 12, fontFamily: FontFamily.medium, textDecorationLine: "underline" },
